...
import ChangePaymentMethodModal from '@/screens/Post/components/forms/modals//ChangePaymentMethodModal'
...
const BasketScreen = ({ navigation, route }) => {
  ...
  if (offerData) {
    return (
      <View>
        <AppText>Replace with need offer basket</AppText>
      </View>
    )
  }
        ...
        schedule:
          postType === 'service'
            ? is_multiple
              ...
              : moment(
                  `${screenParams.schedule?.date} @${screenParams.schedule?.time}`,
                  'MMMM D, YYYY @h:mm a'
                ).toDate()
            : '',
      },
